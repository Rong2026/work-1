            ...
            for mode in ["train","test"]:
                ...
                if len(defect_classes) == 1 and "good" in defect_classes and mode=="train":
                    ...
                    for i in range(len(data_list)):
                        ...
                        save_img_path = os.path.join(des_path_root,new_class_name,"train","good","mvtecloco_"+str(id).zfill(6)+".png") 
                        ...
                        for classes in defect_classes:
                            ...
                            if classes == "good":
                                for i in range(len(data_list)):
                                    ...
                                    save_img_path = os.path.join(des_path_root,new_class_name,"test","good","mvtecloco_"+"good_"+str(id).zfill(6)+".png") 
                                    cv2.imwrite(save_img_path,raw_img)
                                    id = id + 1
                            else:
                                for i in range(len(data_list)):
                                    # 从文件名中提取编号（例如 003.png -> 003）
                                    file_number = os.path.basename(data_list[i]).replace(".png","")
                                    raw_mask_path = os.path.join(data_path,"ground_truth",classes,file_number,"000.png")
                                    raw_img = cv2.imread(data_list[i],cv2.IMREAD_COLOR)
                                    raw_mask = cv2.imread(raw_mask_path,cv2.IMREAD_GRAYSCALE)
                                    if binary:
                                    ...
                                    save_img_path = os.path.join(des_path_root,new_class_name,"test","anomaly",f"mvtecloco_{classes}_"+str(id).zfill(6)+".png")
                                    save_mask_path = os.path.join(des_path_root,new_class_name,"ground_truth","anomaly",f"mvtecloco_{classes}_"+str(id).zfill(6)+".png")

                                    cv2.imwrite(save_img_path,raw_img)
                                    #cv2.imwrite(save_mask_path,raw_mask, [int(cv2.IMWRITE_PNG_COMPRESSION),0])
                                    cv2.imwrite(save_mask_path,raw_mask)
                                    id = id + 1
        print("mvtecloco finished !")
        return id