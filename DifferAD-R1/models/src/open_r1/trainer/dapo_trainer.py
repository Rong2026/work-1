...
class VLMDAPOTrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.beta == 0.0:
            ...
            self.ref_model = model_cls.from_pretrained(model_id, **model_init_kwargs) #Zero-3会吧模型参数拆分到不同的GPU上，不能直接复制已有的model，必须初始化。
        elif peft_config is None:
            # If PEFT configuration is not provided, create a reference model based on the initial model.
            self.ref_model = create_reference_model(model) #一般是直接复制一个，深拷贝或冻结参数
        else:
        ...
        # Reward processing class，确保每个reward function都有一个对应的reward processing class，并正确设置tokneizer
        ...
        for i, (reward_processing_class, reward_func) in enumerate(zip(reward_processing_classes, reward_funcs)):
            if isinstance(reward_func, PreTrainedModel): #若reward_func是一个预训练模型，则进行特殊处理，否则可能只是个普通函数，则不需要tokenizer
                if reward_processing_class is None:
                    reward_processing_class = AutoTokenizer.from_pretrained(reward_func.config._name_or_path)
                if reward_processing_class.pad_token_id is None:
                    reward_processing_class.pad_token = reward_processing_class.eos_token #将pad_token设置为eos_token
        ...
        self.dynamic_sampling      = getattr(args, "dynamic_sampling", False)   # 过滤零方差组
        self.overlong_filter       = getattr(args, "overlong_filter", True)    # 截断样本不回传梯度
        ...
        self.length_cache          = getattr(args, "length_cache", 4096)       # 论文用 16384 + 4096
        
        # 正常样本的难度感知权重系数（微弱调整）
        self.normal_weight_coef    = getattr(args, "normal_weight_coef", 0.5)  # 正常样本难度感知系数，默认0.1

        # DAPO 建议移除 KL 项
        self.beta = 0.0  # 强制关闭 KL；或要求在传入的 args 里把 beta 设为 0
        self.ref_model = None

        # # ===== Dynamic reward weight adjustment based on IoU performance =====
        # self.dynamic_reward_weights = getattr(args, "dynamic_reward_weights", False)
        # if self.dynamic_reward_weights:
        #     self.dynamic_weight_iou_low_threshold = getattr(args, "dynamic_weight_iou_low_threshold", 0.1)
        #     self.dynamic_weight_iou_high_threshold = getattr(args, "dynamic_weight_iou_high_threshold", 0.3)
        #     self.dynamic_weight_center_low = getattr(args, "dynamic_weight_center_low", 0.3)
        #     self.dynamic_weight_center_high = getattr(args, "dynamic_weight_center_high", 1.0)
        #     self.dynamic_weight_iou_low = getattr(args, "dynamic_weight_iou_low", 0.3)
        #     self.dynamic_weight_iou_high = getattr(args, "dynamic_weight_iou_high", 1.0)
            
        #     # Identify reward function indices
        #     self.iou_reward_idx = None
        #     self.center_area_reward_idx = None
        ...
        #         if "iou_reward" in reward_func_name.lower():
        #             self.iou_reward_idx = i
        #         elif "center_area_reward" in reward_func_name.lower() or "center_ares" in reward_func_name.lower():
        #             self.center_area_reward_idx = i
            
        #     if self.iou_reward_idx is None or self.center_area_reward_idx is None:
        #         if self.is_world_process_zero():
        #             print(f"Warning: Could not identify reward function indices for dynamic weight adjustment.")
        #             print(f"  iou_reward_idx: {self.iou_reward_idx}, center_area_reward_idx: {self.center_area_reward_idx}")
        #             print(f"  Reward function names: {[getattr(rf, '__name__', str(rf)) if not isinstance(rf, PreTrainedModel) else rf.config._name_or_path.split('/')[-1] for rf in self.reward_funcs]}")
        #         self.dynamic_reward_weights = False  # Disable if we can't identify the functions
    ...
    def _enable_gradient_checkpointing(self, model: PreTrainedModel, args: GRPOConfig) -> PreTrainedModel:
        ...
        # Enable gradient checkpointing for non-PEFT models
            ...
            try:
                model.gradient_checkpointing_enable() #梯度检查点开启，用计算换显存，不保存所有中间激活，在反向传播时，需要的那部分前向会重新计算一次。
            except:
    ...
    def _get_per_token_logps(self, model, input_ids, attention_mask, **custom_multimodal_inputs):
        ...
        for logits_row, input_ids_row in zip(logits, input_ids):
            log_probs = logits_row.log_softmax(dim=-1) #获取logprobs
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        images = []
        for x in inputs:
            if "image" in x: #也可以返回image，这里直接用image
        ...
        prompt_inputs = self.vlm_module.prepare_model_inputs(
            ...
            return_tensors="pt",
            padding=True,
            padding_side="left", #左填充，便于截断时保留末尾
            add_special_tokens=False,
        )
        prompt_inputs = super()._prepare_inputs(prompt_inputs)

        prompt_ids, prompt_mask = prompt_inputs["input_ids"], prompt_inputs["attention_mask"]
        #prompt_ids形如 (B, P)，B 批大小，P 是 prompt 序列长度；prompt_mask同形状的attention mask（1表示有效位置）

        # Generate completions
        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model: #拿到.genertate的裸模型
            ...
            if not self.vlm_module.is_embeds_input():
                prompt_completion_ids = generate_returned_result #generate返回拼接后的[prompt+completion]的ids,用prompt_length把completion切出来。
        ...
        eos_idx = torch.full((is_eos.size(0),), is_eos.size(1), dtype=torch.long, device=device) #用来存储每个样本中第一个EOS token的索引
        eos_idx[is_eos.any(dim=1)] = is_eos.int().argmax(dim=1)[is_eos.any(dim=1)] #找到每个batch第一个eos_token_id索引的位置，对应为（batch_size,）
        sequence_indices = torch.arange(is_eos.size(1), device=device).expand(is_eos.size(0), -1)
        completion_mask = (sequence_indices <= eos_idx.unsqueeze(1)).int()
        '''
        形如：completion_mask = [
                                [1, 1, 1, 0, 0], # 样本 0: 0,1,2 有效
                                [1, 1, 1, 1, 1]  # 样本 1: 0,1,2,3,4 全部有效 (直到序列末尾，因为EOS在最后)
                                ]，completion_mask在后续计算logprob或损失时非常有用。确保模型只对实际生成的有效部分进行计算，忽略掉EOS之后的无关部分
        '''

        # --- Overlong filtering & soft punishment (DAPO §3.4) ---
        has_eos = is_eos.any(dim=1)
        truncated = ~has_eos  # 未遇到 EOS，说明被 max_new_tokens 截断

        # 过长过滤：被截断的样本不回传梯度（只在 overlong_filter=True 时）
        if self.overlong_filter:
            completion_mask = completion_mask.clone()
            completion_mask[truncated] = 0  # 全零 => 该样本不参与损失
        # 计算生成长度（含 EOS 的位置；若截断就取最大长度）
        gen_len = torch.where(has_eos, eos_idx + 1, torch.full_like(eos_idx, completion_ids.size(1)))

        # 软惩罚（可选）：长度超过 length_max - length_cache 的样本施加线性惩罚
        # R_length ∈ {0, (Lmax-L - Lcache)/Lcache, -1}
        if self.soft_overlong_punish:
            Lmax, Lcache = self.length_max, self.length_cache
            # piecewise: 见 DAPO Eq.(13)
        ...
        #新增12_8
        miou_vec = rewards_per_func[:, 0].detach()   # 形状 [B_local]
        # === 基于阈值的“错误”标记，同样是 [B_local] ===
        ...
        # 提取异常状态信息：通过检查 solution (ground truth) 是否为 [0,0,0,0] 来判断
        # 如果 solution 是 [0,0,0,0]，则为正常样本（has_anomaly=0），否则为异常样本（has_anomaly=1）
        has_anomaly_list = []
        for example in inputs:
        ...
        # # ===== Dynamic reward weight adjustment based on IoU performance =====
        # if self.dynamic_reward_weights and self.iou_reward_idx is not None and self.center_area_reward_idx is not None:
        #     # Calculate current mean IoU (using the IoU reward function)
        #     current_mean_iou = rewards_per_func[:, self.iou_reward_idx].mean().item()
            
        #     # Determine weights based on IoU performance
        #     if current_mean_iou < self.dynamic_weight_iou_low_threshold:
        #         # IoU is very low, prioritize center_area_reward
        #         w_center = self.dynamic_weight_center_high
        #         w_iou = self.dynamic_weight_iou_low
        #     elif current_mean_iou >= self.dynamic_weight_iou_high_threshold:
        #         # IoU is high, prioritize iou_reward
        #         w_center = self.dynamic_weight_center_low
        #         w_iou = self.dynamic_weight_iou_high
        #     else:
        #         # IoU is in the middle range, linear interpolation
        #         progress = (current_mean_iou - self.dynamic_weight_iou_low_threshold) / (
        #             self.dynamic_weight_iou_high_threshold - self.dynamic_weight_iou_low_threshold
        #         )
        #         w_center = self.dynamic_weight_center_high - (
        #             self.dynamic_weight_center_high - self.dynamic_weight_center_low
        #         ) * progress
        #         w_iou = self.dynamic_weight_iou_low + (
        #             self.dynamic_weight_iou_high - self.dynamic_weight_iou_low
        #         ) * progress
            
        #     # Apply weights to reward functions
        #     rewards_per_func = rewards_per_func.clone()  # Avoid modifying the original
        #     rewards_per_func[:, self.iou_reward_idx] *= w_iou
        #     rewards_per_func[:, self.center_area_reward_idx] *= w_center
            
        #     # Log the dynamic weights (only on main process)
        #     if self.is_world_process_zero():
        #         self._metrics["dynamic_weight/center_area"].append(w_center)
        #         self._metrics["dynamic_weight/iou"].append(w_iou)
        #         self._metrics["dynamic_weight/mean_iou"].append(current_mean_iou)
        ...
        if self.dynamic_sampling:
            ...
            # std_grouped_rewards 维度是“全局组数”
            zero_grad_groups = (std_grouped_rewards < 1e-8)  # [global_num_groups]
            # 展开到“全局 completion 数”（= 组数 × num_generations）
            # zero_grad_mask_global = zero_grad_groups.repeat_interleave(self.num_generations)  # [global_num_completions]
            # 切到“本进程的 completion 段”
            zero_grad_mask_local = zero_grad_groups[process_slice]  # [len(prompts)]
            # 与已切过的 advantages 对齐
            advantages = advantages * (~zero_grad_mask_local)
        ...
        return {
            ...
            "miou": miou,
            "p_err": p_err_group,
            "has_anomaly": has_anomaly,  # 异常状态标记：[B_local]，1表示异常，0表示正常
        }
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        #     miou = inputs["miou"]
        #     if self.args.miou_adjust_math == 'power': #对高iou进行压制，想压低高iou的权重
